...
3. **Aggressive cleanup of the body**:
   - Strip ALL timestamps (bracketed and bare) via two regexes — `TS_BRACKET` and `TS_BARE`.
   - Repeatedly strip the sender name from the start of the body (handles the duplicate-name case).
   - Strip leading badge words (`T3TG`, `ADMIN`, etc.) using the `BADGE_WORDS` Set.
   - Fallback: `"Name: body"` regex if the DOM-based sender extraction failed.
4. **Apply first-name-only** if `config.firstNameOnly` is true — splits sender on whitespace and keeps only the first word.
...
In `handleRootNode`, the spoken string is assembled from parts:
...
So toggling features just adds/removes parts — no special-case branching.
